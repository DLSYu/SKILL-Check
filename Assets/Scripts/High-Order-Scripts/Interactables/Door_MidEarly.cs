...
public class Door_MidEarly : MonoBehaviour, IInteractable
{
    [Header("Story Data")]
    [TextArea(3, 10)]
    ...
    {
        if (!InventoryManager_MidEarly.Instance)
        {
            Debug.LogError("InventoryManager_Early missing!");
            return;
        }

        if (InventoryManager_MidEarly.Instance.CollectedGemsCount >= requiredGems)
        {
        ...
        {
            Debug.Log($"Need {requiredGems - InventoryManager_MidEarly.Instance.CollectedGemsCount} more gems!");
        }
    }
